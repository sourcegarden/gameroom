...
exports.listen = function(server){
    ...
    io.sockets.on('connection', function(socket){
           ...
           // handleMotionBroadcasting(socket);
            handleTouchBroadcasting(socket);
...
function handleTouchBroadcasting(socket) {
    socket.on('touch', function(message) {
        socket.broadcast.to(currentRoom[socket.id]).emit('touch', {
            room: message.room,
            socketid: socket.id,
            touchKey: message.touchKey,
            bDown: message.bDown
        });
        console.log("client:" + socket.id + " touch : " + message.touchKey + " bDown=" + message.bDown);
    });
}
